...
LoginService.service('loginService',['postService','$cordovaProgress','$timeout','$location', function  (postService, $cordovaProgress, $timeout, $location) {
	// ls = loginService
	var ls = this;
	ls.user = {};
		...
		ls.postService.post(user, ls.url).then(function() {
			...
			if ( ls.response.result == 0){
				...
				$timeout(function() {
			 	...
			 	$timeout(function() {
				         $cordovaProgress.showText(true, "登陆失败: " + ls.error_msg , 'center')
				    }, 3000);
				$timeout(function() {
				        $cordovaProgress.hide()
				    }, 6000);
